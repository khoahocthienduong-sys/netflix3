    ...
    const result = await new Promise((resolve, reject) => {
      imap.once('ready', () => {
        imap.openBox('INBOX', true, (err) => {
          ...
          // Tìm email Netflix từ 7 ngày trước (giữ nguyên như code gốc)
          ...
          imap.search([['FROM', 'netflix'], ['SINCE', since]], (err, results) => {
            ...
            // Lấy email mới nhất (giữ nguyên như code gốc)
            ...
            f.on('message', (msg) => {
              msg.on('body', (stream) => {
                simpleParser(stream, (err, parsed) => {
                  ...
                  // Tìm mã OTP 4-6 chữ số (tìm trong cả text và html)
                  const textContent = parsed.text || '';
                  const htmlContent = parsed.html || '';
                  const codeMatch = textContent.match(/\b(\d{4,6})\b/) || htmlContent.match(/\b(\d{4,6})\b/);
                  const code = codeMatch ? codeMatch[1] : null;

                  // Tìm link nút đỏ từ HTML email
                  // mailparser đã decode quoted-printable → link đầy đủ trong HTML
                  // Chỉ cần decode &amp; → &
